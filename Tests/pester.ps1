...
Remove-Module Daybreak2018 -ErrorAction Ignore
Import-Module "$PSScriptRoot\..\Daybreak2018.psd1"
Import-Module "$PSScriptRoot\..\Daybreak2018.psm1" -Force
...
if ($totalFailed -eq 0) { Write-PSFMessage -Level Critical -Message "All <c='em'>$totalRun</c> tests executed without a single failure!" } else { Write-PSFMessage -Level Critical -Message "<c='em'>$totalFailed tests</c> out of <c='sub'>$totalRun</c> tests failed!" }
